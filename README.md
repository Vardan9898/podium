...
| | |
|---|---|
| Backend checks | Pint · Larastan level 8 · **162 Pest tests** (incl. a 56-case authorization matrix) |
...
10. [Trade-offs & what I'd do next](#trade-offs--what-id-do-next)
...
- **List queries** are composed from `Proposal` scopes — `visibleTo(User)`, `search()`, `withAnyTags()`, `status()`, `withSummary()` — and `visibleTo()` is tested against the `view` policy so the list and the detail endpoint can never disagree.
...
- **`ILIKE` search** is fine at this scale. Next step: a `pg_trgm` GIN index, or Laravel Scout (database driver now, Meilisearch via env later) for relevance ranking.
