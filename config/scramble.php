...
return [
    ...
    'info' => [
         ...
         */
        'description' => <<<'MD'
            JSON API behind the Podium SPA.

            **Auth:** Sanctum SPA cookies. Call `GET /sanctum/csrf-cookie`, then send the `XSRF-TOKEN`
            cookie value back as the `X-XSRF-TOKEN` header with a first-party `Origin`/`Referer`.

            **Errors:** `422` uses Laravel's `{message, errors}` shape; every other error is `{message}`.
            MD,
    ],
